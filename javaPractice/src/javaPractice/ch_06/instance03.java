...
class OutClass_03{
	...
	Runnable getRunnable(int i) {// 반환형이 Runnable인 메서드
		...
		/*
		 ...
		 java.lang 패키지에 선언되어 있으며 반드시 run() 메서드를 구현해야함.
		 * */
		class MyRunnable implements Runnable{
			...
			public void run() {
				/* 지역 내부 클래스가 선언된 메서드의 지역변수는 사용하지 못함.
				 num = 200; // 에러 지역변수는 상수로 바뀜.값은 변경을 못해도 보는것은 가능
				 i = 100; // 에러 매개 변수 역시 지역변수처럼 상수로 바뀜 값은 변경은 못해도 보는 것은 가능.
				 * */
				System.out.println("i =" +i); // 값 변경은 못해도 보는 것은 가능
				System.out.println("num = "+num); // 값 변경은 못해도 보는 것은 가능
