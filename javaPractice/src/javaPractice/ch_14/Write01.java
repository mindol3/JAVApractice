...
public class Write01 {
	...
	Writer 제공 메서드
	void writer(int c) : 한 문자를 파일에 출력
	void writer(char[] buf) : 문자 배열 buf의 내용을 파일에 출력
	void writer(char[] buf, int off, int len) : 문자 배열 buf의 off위치에서 len 개수의 문자를 파일에 출력.
	void writer(string str) : 문자열 str을 파일에 출력
	void writer(string str, int off, int len) : 문자열 str의 off번째 문자부터 len 개수 만큼 파일에 출력.
