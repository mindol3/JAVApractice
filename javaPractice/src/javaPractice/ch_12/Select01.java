...
class DBSelect extends DB {
	...
	protected ResultSet querySelect(String sql) {
		...
		try {
			...
			// SQL 문이 INSERT, UPDATE, DELETE일 경우에는 excuteUpdate () 메소드를 호출하지만,
			// 데이터를 가져오는 SELECT 문일 경우에는 executeQuery() 메소드를 호출.
			resultSet = preparedStatement.executeQuery();
		} catch (SQLException e) {
			System.out.println("error: " + e);
		}
		// ResultSet은 SELECT 문에 기술된 컬럼으로 구성된 행row의 집함.
		return resultSet;
	}
	...
	private void printUser(String sql) throws SQLException {
		...
		if(totalRow == 1) {
			...
			if(resultSet.next()) {
				...
				System.out.println("입력하산 아이디의 데이터는 존재하지 않습니다.");
			}
		}
		else {
			// 전체 목록 출력
			// 데이터의 처움부터 읽어오므로 next를 만나면 한 줄을 읽어오고 , 다음 줄로 커서가 이동.
			// 한줄 한줄 내려감녀서 데이터를 가져오고, 데이터가 끝까지 가면 종료.
	...
	public void getOneUser() throws SQLException {
		...
		do {
			...
			//입력 값이 y가 아닌경우 true, 같은 경우 false 반환
